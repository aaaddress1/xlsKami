...
namespace Macrome
{
    ...
    public class WorkbookEditor
    {
        ...
        public WorkbookStream SetMacroSheetContent_Alert(string szPopup, int rwStart = 0, int colStart = 0)
        {
            WorkbookStream macroStream = GetMacroStream();
            ...
            List<BiffRecord> formulasToAdd = new List<BiffRecord>();
            var k = new Formula(new Cell(rwStart, colStart), FormulaValue.GetEmptyStringFormulaValue(), true, new CellParsedFormula(FormulaHelper.GetAlertPtgStack(szPopup)));
            formulasToAdd.Add(k);

            WbStream = WbStream.InsertRecords(formulasToAdd, replaceMeFormula);
            return WbStream;
        }
